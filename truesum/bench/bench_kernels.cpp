...
// plain, and with TRUESUM_KERNEL=scalar -- and compare. TRUESUM_SPAN moves the
// per-block dispatch threshold of the AVX-512 kernel (see kernels_avx512.cpp);
// a very large value disables the dispatch, which is how its effect is
// isolated.
//
...
// bulk sits within 16 binades, which is the shape the dispatch must not
// misjudge.
//
// The per-block dispatch this was written to evaluate is not in the library;
// see docs/simd-design.md for what it measured and why it was rejected. The
// TRUESUM_SPAN knob it used is gone with it.
//
// Usage: bench_kernels
//
// Until the benchmark targets land, build it standalone:
//   g++ -O2 -std=c++17 -Iinclude bench/bench_kernels.cpp build/libtruesum.a \
//       -lpthread -o bench_kernels
