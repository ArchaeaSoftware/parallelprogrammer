...
Sizing a column that way requires knowing the extents of an input before any of it is applied, which is what a survey supplies: it determines whether any column must be rescaled and whether new limb columns must be allocated. The library takes that survey itself unless it was handed surveys up front. Separating the survey from the accumulation is what makes distributed accumulation practical. The nodes that computed the matrices can compute their surveys too and transmit them ahead to the accumulation node; once it has been pre-sized to receive them all, the matrices themselves follow, one matrix or even one column at a time, in any order. For any order, the final output is bit-identical.
