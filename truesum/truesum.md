...
When reading matrix data from mapped pinned memory, the developer must take care to avoid write-after-read race conditions, since the kernel will still be streaming its input after the call that submitted it has returned. A submission is a kernel launch on the accumulation matrix's stream, so the ordinary CUDA answer applies: record an event on that stream afterwards, and it completes when the kernel does. `stream()` returns the stream for exactly that. A producer with two buffers in rotation keeps one event per buffer and waits only for the one it is about to refill, never for the stream as a whole.

A producer can avoid the events altogether by queuing its copies on that same stream, which then orders each one against the accumulate before it and the one after it: a single buffer can be refilled in place, with nothing to wait on. The stream may be the caller's own, supplied to the constructor, which is how to keep the overlap that the library's default gives up for safety. A two-buffer rotation on a caller's stream costs 1301 µs a batch at 65536 × 64 against 1256 µs for the fill alone, so the accumulate is 96 percent hidden.
