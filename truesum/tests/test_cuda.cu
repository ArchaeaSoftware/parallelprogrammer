...
//
// Same assertion-runner style as tests/test_truesum.cpp, no dependencies.
...
template <typename Gen>
void
cross_check_symmetric(const char *name, std::size_t n, truesum::Uplo uplo,
...
test_symmetric()
{
    ...
    cross_check_symmetric(
        "sym 512 lower, 3 batches", 512, truesum::Uplo::Lower, 3,
        [](std::size_t, std::size_t, int) { return random_value(8); });
    cross_check_symmetric(
        "sym 512 upper, 3 batches", 512, truesum::Uplo::Upper, 3,
        [](std::size_t, std::size_t, int) { return random_value(8); });
    cross_check_symmetric(
        "sym 257 lower ragged, 2 batches", 257, truesum::Uplo::Lower, 2,
        [](std::size_t, std::size_t, int) { return random_value(16); });
    cross_check_symmetric(
        "sym 129 upper ragged, 2 batches", 129, truesum::Uplo::Upper, 2,
        [](std::size_t, std::size_t, int) { return random_value(16); });
    ...
    cross_check_symmetric(
        "sym 128 lower wide spread, 3 batches", 128, truesum::Uplo::Lower, 3,
        [](std::size_t, std::size_t, int) { return random_value(400); });

    // Degenerate sizes, where the last column holds a single entry.
    cross_check_symmetric(
        "sym 1", 1, truesum::Uplo::Lower, 2,
        [](std::size_t, std::size_t, int) { return random_value(4); });
    cross_check_symmetric(
        "sym 8 upper", 8, truesum::Uplo::Upper, 2,
        [](std::size_t, std::size_t, int) { return random_value(4); });
...
test_device_survey()
{
    ...
    std::vector<truesum::Survey> host(cols), device(cols);
    truesum::survey_matrix_col_major(host.data(), src.data(), rows, cols);

    // The device writes `out`, so it goes to device memory and comes back by
    // an explicit copy the caller chose to make.
    truesum::Survey *d_out = nullptr;
    cudaMalloc(&d_out, cols * sizeof(truesum::Survey));
    truesum::survey_matrix_col_major_device(d_out, dev, rows, cols);
    cudaMemcpy(device.data(), d_out, cols * sizeof(truesum::Survey),
    ...
    {
        std::vector<truesum::Survey> pageable(cols);
        bool threw = false;
        try {
            truesum::survey_matrix_col_major_device(pageable.data(), dev, rows,
                                                 cols);
        } catch (const std::invalid_argument &) {
            threw = true;
        }
